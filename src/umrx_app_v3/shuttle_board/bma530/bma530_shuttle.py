...
from umrx_app_v3.sensors.bma530 import BMA530, BMA530Addr, BMA530ExtendedAddr

logger = logging.getLogger(__name__)


class BMA530ShuttleError(Exception): ...
...
class BMA530Shuttle:
    ...
    def check_connected_hw(self) -> None:
        ...
        if board_info.shuttle_id != self.SHUTTLE_ID:
            ...
            raise BMA530ShuttleError(error_message)
    ...
    def read_register(self, reg_addr: int, bytes_to_read: int = 1) -> array[int] | int:
        if isinstance(reg_addr, BMA530Addr | BMA530ExtendedAddr):
    ...
    def write_register(self, reg_addr: int, value: int | array) -> None:
        ...
        if self.is_i2c_configured:
            if isinstance(value, array):
                return self.board.write_i2c(self.I2C_DEFAULT_ADDRESS, reg_addr, value)
    ...
    def _configure_i2c_polling_streaming(
        ...
        self.board.streaming_polling_set_i2c_channel(
            ...
            register_address=BMA530Addr.acc_data_0.value,
            bytes_to_read=(6 + 1 + 3),
        )
    ...
    def _configure_i2c_interrupt_streaming(self) -> None:
        self.board.streaming_interrupt_set_i2c_channel(
            ...
            i2c_address=BMA530Shuttle.I2C_DEFAULT_ADDRESS,
            register_address=BMA530Addr.acc_data_0.value,
            bytes_to_read=(6 + 1 + 3),
        )
    ...
    def configure_interrupt_streaming(self) -> None:
        self.switch_on_accel()
        self.sensor.int1_conf = (0b10 << 0) | (0b1 << 3)
        self.sensor.int_map_0 = 0b01 << 0
        self.sensor.int2_conf = 0x01
        time.sleep(0.02)
        if self.is_i2c_configured:
