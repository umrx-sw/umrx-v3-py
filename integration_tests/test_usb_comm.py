...
def test_usb_comm_construction(usb_comm: UsbCommunication) -> None:
    assert usb_comm.vid == 0x152A, "Expected VID & PID for App Board 3.0 wrong"
    assert usb_comm.pid == 0x80C0, "Expected VID & PID for App Board 3.0 wrong"
...
@pytest.mark.usb_comm
def test_usb_comm_endpoint_packet(usb_comm: UsbCommunication) -> None:
...
@pytest.mark.usb_comm
def test_usb_comm_recv(usb_comm: UsbCommunication) -> None:
    data = usb_comm.receive()
    assert isinstance(data, array), "Expecting `array` of bytes back"


@pytest.mark.usb_comm
def test_usb_comm_send(usb_comm: UsbCommunication) -> None:
